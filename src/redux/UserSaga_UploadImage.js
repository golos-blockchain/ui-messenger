...
function* uploadImage(action) {
    ...
    if (!$GLS_Config.images || !$GLS_Config.images.upload_image) {
    ...
    let postUrl = $GLS_Config.images.upload_image
    ...
    if (file && file.size > MAX_UPLOAD_IMAGE_SIZE) {
        if (useGolosImages) {
            ...
            postUrl = new URL('/@' + username + '/' + signatures.posting, $GLS_Config.images.img_proxy_prefix).toString();
            golosImages = true
        } else {
    ...
    if (!golosImages && $GLS_Config.images.client_id) {
