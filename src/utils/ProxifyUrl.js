 ...
 * @param {string} url
 * @param {string|boolean} dimensions - optional -  if provided. url is proxied && global var $GLS_Config.images.img_proxy_prefix is avail. resp will be "$GLS_Config.images.img_proxy_prefix{dimensions}/{sanitized url}"
...
export const proxifyImageUrlWithStrip = (url, dimensions = false) => {
  ...
  if (dimensions && $GLS_Config && $GLS_Config.images && ($GLS_Config.images.img_proxy_prefix || $GLS_Config.img_proxy_backup_prefix)) {
    let dims = dimensions + '/';
